...
func various() {
	// Переменная представляет собой область памяти, в которой хранится значение. Чтобы к переменной можно было обращаться по имени,
	//используйтся объявление переменной. Объявление состоит из ключевого слова var, за которым следует имя и тип значений,
	// которые будут храниться в переменной.
